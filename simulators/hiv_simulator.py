...
class HIVSimulator(object):
    ...
    **REFERENCE:**
    ...
    def __init__(self, logspace=True, dt=5, model_derivatives=None, perturb_params=True, p_init = np.zeros(6),
        p_T1=0, p_T2=0, p_T1s=0, p_T2s=0, p_V=0, p_E=0, perturb_bounds=(-0.05, 0.05), **kw):
        """
        ...
        self.p_init = p_init
        self.perturb_bounds = perturb_bounds
    ...
    def reset(self, perturb_params=True):
        ...
        if not perturb_params:
            ...
            pert = np.random.uniform(*self.perturb_bounds, size=6)
            self.state = self.baseline_state + (self.baseline_state * pert)
